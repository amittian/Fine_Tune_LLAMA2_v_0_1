...
import pandas as pd
import plotly.io as FUZE_pio

# Generate HTML strings for DataFrames
FUZE_topic_info_html = topic_modeling.get_topic_info().to_html()
FUZE_hierarchical_topics_html = hierarchical_topics.to_html()

# Generate HTML strings for Plotly visualizations
FUZE_fig_barchart = topic_modeling.topic_model.visualize_barchart()
FUZE_barchart_html = FUZE_pio.to_html(FUZE_fig_barchart, full_html=False)

FUZE_fig_heatmap = topic_modeling.topic_model.visualize_heatmap()
FUZE_heatmap_html = FUZE_pio.to_html(FUZE_fig_heatmap, full_html=False)

FUZE_fig_topics = topic_modeling.topic_model.visualize_topics()
FUZE_topics_html = FUZE_pio.to_html(FUZE_fig_topics, full_html=False)
...
<html>
<head>
    <title>Comprehensive FUZE C-Ops Topic Modeling Visualization</title>
    <style>
        body {{
...
<body>
    <h1>FUZE C-Ops Bar Chart Visualization</h1>
    {FUZE_barchart_html}
    <h1>FUZE C-Ops Plotly Heatmap Visualization</h1>
    {FUZE_heatmap_html}
    <h1>FUZE C-Ops Hierarchical Topics Visualization</h1>
    {FUZE_hierarchical_topics_html}
    <h1>FUZE C-Ops Topic Visualization</h1>
    {FUZE_topics_html}
    <h1>FUZE C-Ops Topic Information</h1>
    {FUZE_topic_info_html}
</body>
</html>
"""

# Write the combined HTML content to a file
with open('combined_FUZE_topic_visualization_set_01.html', 'w') as FUZE_file:
    FUZE_file.write(FUZE_html_content)

print("All visualizations have been saved in 'combined_FUZE_topic_visualization_set_01.html'.")
...
FUZE_df_token_distro = topic_modeling.topic_model.visualize_approximate_distribution(docs[1], topic_token_distr[1])

# Convert the Styler object to HTML
FUZE_token_distro_html = FUZE_df_token_distro.to_html()

# Step 3: Visualize the term rank (returns a Plotly figure)
FUZE_fig_term_rank = topic_modeling.topic_model.visualize_term_rank()

# Convert the Plotly figure to an HTML string
FUZE_term_rank_html = pio.to_html(FUZE_fig_term_rank, full_html=False)

# Step 4: Combine the visualizations into a single HTML file
FUZE_html_content = f"""<!DOCTYPE html>
<html>
<head>
    <title>FUZE C-Ops Comprehensive Visualization</title>
    <style>
        body {{
            font-family: Arial, sans-serif;
            margin: 20px;
...
<body>
    <h1>FUZE C-Ops Token-Level Distribution</h1>
    {FUZE_token_distro_html}  <!-- Display the token-level distribution as a table -->
    <h1>FUZE C-Ops Term Rank Visualization</h1>
    {FUZE_term_rank_html}  <!-- Display the Plotly term rank figure -->
</body>
</html>
"""

# Step 5: Write the final combined HTML content to a file
with open('FUZE_token_distribution_and_term_rank_set_03.html', 'w', encoding='utf-8') as FUZE_file:
    FUZE_file.write(FUZE_html_content)

print("Both visualizations have been saved in 'FUZE_token_distribution_and_term_rank_set_03.html'.")
